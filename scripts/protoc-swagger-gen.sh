...
declare -a swagger_fixes=(
  "./tmp-swagger-gen/cosmos/tx/v1beta1/service.swagger.json::cosmos.tx.v1beta1.ModeInfo.Multi.properties.mode_infos.items"
  "./tmp-swagger-gen/cosmos/autocli/v1/query.swagger.json::cosmos.autocli.v1.ServiceCommandDescriptor.properties.sub_commands"
)

for fix in "${swagger_fixes[@]}"; do
  IFS="::" read -r file json_path <<< "$fix"
  jq "del(.definitions[\"$json_path\"])")" "$file" > "${file%.json}_fixed.json"
  rm -f "$file"
done
